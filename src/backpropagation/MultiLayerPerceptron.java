    ...
    public double backPropagate(Entrada entradas) {
        ...
        for (int k = capas.size() - 2; k >= 0; k--) {
            ...
            for (int i = 0; i < capas.get(k).layerSize(); i++) {
                ...
                evaluated = capas.get(k).getNeuron(i).getFuncionTransferencia()
                        .evaluateDerivate(capas.get(k).getNeuron(i).getValue());
            ...
            for (int i = 0; i < capas.get(k + 1).layerSize(); i++) {
                for (int j = 0; j < capas.get(k).layerSize(); j++) {
                    ...
                    capas.get(k + 1).getNeuron(i).setWeight(j, varWeight + (tasaAprendizaje
                            * capas.get(k + 1).getNeuron(i).getDelta() * capas.get(k).getNeuron(j).getValue()));
                }
                double varThreshold = capas.get(k + 1).getNeuron(i).getThreshold();
                capas.get(k + 1).getNeuron(i)
                        .setThreshold(varThreshold + (tasaAprendizaje * capas.get(k + 1).getNeuron(i).getDelta()));
            }
        }
